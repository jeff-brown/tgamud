...
class MudServer(object):
    ...
    def update(self):
        ...
        if self._events:
            print("update events: {}".format(self._events))
        self._new_events = []
    ...
    def _check_for_messages(self):
        ...
        # go through all the clients
        for pid, cl in list(self._clients.items()):
            ...
            rlist, _, _ = select.select([cl.socket], [], [], 0)
...
if __name__ == '__main__':
    ...
    while True:
        ...
        for pid in mud.get_new_players():
            ...
            mud.send_message(pid, "What is your name?")

        # go through any new commands sent from players
        for cid, command, params in mud.get_commands():
            print("The cid is {}, command is {} and params are {}.".format(cid, command, params))
            ...
            if players[cid]["name"] is None:

                players[cid]["name"] = command
                players[cid]["room"] = "Tavern"
                ...
                mud.send_message(cid, "Welcome to the game, {}. ".format(
                                                               players[cid]["name"])
                                 + "Type 'help' for a list of commands. Have fun!")

                # send the new player the description of their current room
                mud.send_message(
                    cid, rooms[players[pid]["room"]]["description"])

            # 'look' command
            elif command == "look":

                # store the player's current room
                rm = rooms[players[cid]["room"]]

                # send the player back the description of their current room
                mud.send_message(cid, rm["description"])

                playershere = []
                # go through every player in the game
                for pid, pl in players.items():
                    # if they're in the same room as the player
                    if players[pid]["room"] == players[cid]["room"]:
                ...
                mud.send_message(cid, "Players here: {}".format(
                                                        ", ".join(playershere)))

                # send player a message containing the list of exits from this room
                mud.send_message(cid, "Exits are: {}".format(
                                                        ", ".join(rm["exits"])))

            # some other, unrecognised command
            else:
                # send back an 'unknown command' message
                # mud.send_message(pid, "Unknown command '{}'".format(command))
                for pid, pl in players.items():
                    # send each player a message to tell them about the new player
                    if pid != cid:
                        mud.send_message(pid, "{} said {}.".format(players[cid]["name"], command))
